          ...
          return (
            <div key={item.id} className="relative">
                            ...
                            ${equipped
                              ...
                              : isPreviewing
                                ...
                                : unlocked
                                  ...
                                  : isPremium
                                    ? "border-sf-gold/30 bg-gradient-to-b from-sf-gold-light to-sf-bg cursor-default"
              ...
              >
                ...
                <div className={`text-[9px] mt-0.5 leading-tight truncate ${isPremium && !unlocked ? "text-sf-gold font-medium" : "text-sf-text-secondary"}`}>
                  {t(item.nameKey)}
                </div>
                {/* Lock badge (tier-locked) */}
                {!unlocked && !isPremium && (
                  <div className="absolute -top-1 -right-1 text-[8px] font-bold rounded-full px-1 py-0.5 leading-none bg-sf-text text-white">
                ...
                {isPremium && !unlocked && !isPreviewing && (
                  <div className="absolute -top-1 -right-1 text-[7px] font-bold rounded-full px-1.5 py-0.5 leading-none bg-sf-gold text-white shadow-sm">
                    {item.price}&nbsp;&euro;
                  </div>
                )}
                {/* Equipped check */}
              ...
              {isPremium && !equipped && (
                <button
                  ...
                  className={`absolute -bottom-1 -right-1 w-5 h-5 rounded-full flex items-center justify-center transition-all shadow-sm
                              ${isPreviewing
